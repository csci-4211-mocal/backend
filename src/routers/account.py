...
async def get_account_data(authorization: Authorization):
    ...
    if account is None:
        return HTTPException(status.HTTP_404_NOT_FOUND, "No account found")

    refresh_token = generate_token(account_id)
    if refresh_token is None:
        return HTTPException(status.HTTP_500_INTERNAL_SERVER_ERROR, "Could not generate refresh token")
...
async def login(credentials: Credentials):
    ...
    if found_account is not None:
        if password != found_account.password:
            return HTTPException(status.HTTP_401_UNAUTHORIZED, "Wrong username or password")
